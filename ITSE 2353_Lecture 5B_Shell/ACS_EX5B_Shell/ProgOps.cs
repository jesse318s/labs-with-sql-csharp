...
namespace ACS_EX5B_Shell
{
    class ProgOps
    {
        ...
        private static SqlConnection _cntBooksDatabase = new SqlConnection(CONNECT_STRING);
        ...
        public static void TitlesCommand(TextBox txTitle, TextBox txYear, MaskedTextBox txISBN, 
            TextBox txDesc, TextBox txNotes, TextBox txSubject, TextBox txComments, ComboBox cbPublishers)
        {
            try
            {
                //statement for the command string
                string sqlStatement = "Select * From jsitessp222353.Titles ORDER BY Title";
                ...
                string sqlStatementP = "Select * From jsitessp222353.Publishers ORDER BY Name";
                ...
                cbPublishers.DataSource = _dtPublishersTable;
                cbPublishers.DisplayMember = "Name";
                cbPublishers.ValueMember = "PubID";
                cbPublishers.DataBindings.Add("SelectedItem", _dtPublishersTable, "PubID");
                //cbPublishers.DataBindings.Add("SelectedItem", _dtTitlesTable, "PubID");
            }
            catch (SqlException ex)
            {
                if (ex is SqlException)
        ...
        public static void DisposePub()
        {
            ...
            catch (SqlException ex)
            {
                ...
                {//handles more specific SqlException here.
                    ...
                    MessageBox.Show(_errorMessages.ToString(), "Error Dispose Publisher", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {//handles generic ones here
                    MessageBox.Show(ex.Message + "Error (PO8)", "Error Dispose Publisher", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
