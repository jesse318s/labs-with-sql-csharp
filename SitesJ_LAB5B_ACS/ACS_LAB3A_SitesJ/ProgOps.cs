...
namespace ACS_LAB3A_SitesJ
{
    class ProgOps
    {
        ...
        private static SqlConnection _cntDatabase = new SqlConnection("Data Source= (LocalDB)\\MSSQLLocalDB;" + 
            "AttachDbFilename= " + Application.StartupPath + "\\SQLNwindDB.mdf; Integrated Security=True;" +
            "Connect Timeout=30");
        //add cmd object
        ...
        public static void UpdateOnCloseSup()
        {
            try
            {
                //save the updated suppliers table
                SqlCommandBuilder suppliersAdapterCommands = new SqlCommandBuilder(_daSuppliers);
                _daSuppliers.Update(_dtSuppliersTable);
            }
            catch (SqlException ex)
            {
                if (ex is SqlException)
            ...
            TextBox tbShipPostalCode, TextBox tbShipCountry)
        {
            ...
            {
                //string to build the query
                string query = "SELECT * FROM Orders WHERE EmployeeID = " + employeeID + "ORDER BY CustomerID";
                //establish command object
